...
describe('run-ci', () => {
  ...
  it('serializes generated checks and avoids pnpm for direct preflight gates', () => {
    ...
    expect(source).toContain("tsxScript('docs integrity check', 'scripts/check-docs-integrity.ts')")
    expect(source).not.toContain("tsxScript('docs discovery check', 'scripts/check-docs-discovery.ts')")
    expect(source).toContain("await runSequential('static direct checks'")
    expect(source).toContain("direct('source size check', 'bun', 'scripts/check-source-file-size.ts')")
    expect(source).toContain("direct('test size check', 'bun', 'scripts/check-test-file-size.ts')")
  ...
  it('runs the CI orchestrator through tsx instead of bun', () => {
    ...
    expect(scripts['docs:discovery:check']).toContain('tsx scripts/check-docs-integrity.ts')
    expect(scripts['docs:discovery:check']).not.toContain('scripts/check-docs-discovery.ts')
    expect(scripts['docs:discovery:full']).toContain('tsx scripts/check-docs-discovery.ts')
    expect(scripts['test-size:check']).toBe('bun scripts/check-test-file-size.ts')
    expect(scripts['analyze:quality']).toContain('pnpm test-size:check')
    expect(scripts['test:semantic:fast']).toContain('pnpm test-size:check')
