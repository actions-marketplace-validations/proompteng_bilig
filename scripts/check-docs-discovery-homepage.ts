...
export function requireHomepageDiscovery(index: string, siteCss: string, productCss: string, docsRoot: string): void {
  ...
  requireAllIncludes(
    ...
    [
      ...
      'The checked artifact compares WorkPaper against HyperFormula on deterministic workbook workloads.',
      'Coverage boundaries stay visible.',
      'workpaper-vs-hyperformula.json',
      'HyperFormula',
      'mean+p95 rows',
      'pnpm workpaper:bench:competitive:check',
      `<strong>${benchmarkEvidence.meanWinHeadline}</strong>`,
      `${benchmarkEvidence.meanAndP95Headline} comparable workloads win on both mean and p95 in the checked HyperFormula artifact.`,
