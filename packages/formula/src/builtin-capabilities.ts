import { BUILTINS, BuiltinId } from '@bilig/protocol'
import { normalizeFormulaFunctionName } from './function-name-normalization.js'

export type BuiltinCapabilityCategory =
  | 'aggregation'
  | 'logical'
  | 'information'
  | 'text'
  | 'date-time'
  | 'lookup-reference'
  | 'statistical'
  | 'dynamic-array'
  | 'lambda'
  | 'math'

export type BuiltinJsStatus = 'implemented' | 'special-js-only'
export type BuiltinWasmStatus = 'production' | 'not-started'

export interface BuiltinCapability {
  readonly id?: BuiltinId
  readonly name: string
  readonly category: BuiltinCapabilityCategory
  readonly jsStatus: BuiltinJsStatus
  readonly wasmStatus: BuiltinWasmStatus
  readonly needsMetadata: boolean
  readonly needsArrayRuntime: boolean
  readonly needsExternalAdapter: boolean
}

const jsSpecialBuiltinNames = new Set([
  'ARRAYFORMULA',
  'LET',
  'LAMBDA',
  'MAKEARRAY',
  'MAP',
  'REDUCE',
  'SCAN',
  'BYROW',
  'BYCOL',
  'CELL',
  'COLUMN',
  'FORMULA',
  'FORMULATEXT',
  'ISFORMULA',
  'GROUPBY',
  'INDIRECT',
  'GETPIVOTDATA',
  'MULTIPLE.OPERATIONS',
  'PIVOTBY',
  'ROW',
  'SHEET',
  'SHEETS',
  'SPLIT',
  'QUERY',
])

const wasmProductionBuiltinAliases = new Set(['AVERAGE', 'FORECAST.LINEAR', 'USE.THE.COUNTIF'])
const wasmProductionBuiltinNames = new Set([
  ...BUILTINS.filter((builtin) => builtin.supportsWasm && !builtin.name.startsWith('__')).map((builtin) => builtin.name.toUpperCase()),
  ...wasmProductionBuiltinAliases,
])

const aggregationBuiltinNames = new Set([
  'SUM',
  'AVERAGE',
  'AVG',
  'MIN',
  'MAX',
  'COUNT',
  'COUNTA',
  'COUNTBLANK',
  'COUNTUNIQUE',
  'COUNTUNIQUEIFS',
])
const logicalBuiltinNames = new Set(['IF', 'IFS', 'AND', 'OR', 'NOT', 'SWITCH', 'XOR', 'IFERROR', 'IFNA', 'NA'])
const informationBuiltinNames = new Set([
  'CELL',
  'FORMULA',
  'FORMULATEXT',
  'ISFORMULA',
  'ISBLANK',
  'ISERR',
  'ISERROR',
  'ISLOGICAL',
  'ISNA',
  'ISNONTEXT',
  'ISNUMBER',
  'ISTEXT',
  'ISEMAIL',
  'ISURL',
  'N',
  'SHEET',
  'SHEETS',
  'T',
  'TYPE',
])
const textBuiltinNames = new Set([
  'LEN',
  'CONCAT',
  'REPLACE',
  'SUBSTITUTE',
  'REPT',
  'EXACT',
  'TEXTBEFORE',
  'TEXTAFTER',
  'TEXTJOIN',
  'SPLIT',
  'LEFT',
  'RIGHT',
  'MID',
  'LEFTB',
  'RIGHTB',
  'MIDB',
  'TRIM',
  'UPPER',
  'LOWER',
  'FIND',
  'SEARCH',
  'FINDB',
  'LENB',
  'SEARCHB',
  'REPLACEB',
  'VALUE',
  'CHAR',
  'CODE',
  'UNICODE',
  'UNICHAR',
  'CLEAN',
  'ASC',
  'JIS',
  'DBCS',
  'BAHTTEXT',
  'PHONETIC',
  'NUMBERVALUE',
  'TEXT',
  'VALUETOTEXT',
  'DOLLAR',
  'ARRAYTOTEXT',
  'REGEXMATCH',
  'JOIN',
  'TO_TEXT',
])
const dateTimeBuiltinNames = new Set([
  'DATE',
  'YEAR',
  'MONTH',
  'DAY',
  'TIME',
  'HOUR',
  'MINUTE',
  'SECOND',
  'WEEKDAY',
  'ISOWEEKNUM',
  'TODAY',
  'NOW',
  'RAND',
  'EDATE',
  'EOMONTH',
  'DATEDIF',
  'DAYS',
  'DAYS360',
  'TIMEVALUE',
  'WEEKNUM',
  'WORKDAY',
  'NETWORKDAYS',
  'WORKDAY.INTL',
  'NETWORKDAYS.INTL',
  'YEARFRAC',
  'EFFECT',
  'NOMINAL',
  'PDURATION',
  'RRI',
  'PV',
  'PMT',
  'NPER',
  'NPV',
  'RATE',
  'IPMT',
  'PPMT',
  'ISPMT',
  'CUMIPMT',
  'CUMPRINC',
  'IRR',
  'MIRR',
  'XNPV',
  'XIRR',
  'DISC',
  'INTRATE',
  'RECEIVED',
  'COUPDAYBS',
  'COUPDAYS',
  'COUPDAYSNC',
  'COUPNCD',
  'COUPNUM',
  'COUPPCD',
  'PRICEDISC',
  'YIELDDISC',
  'PRICEMAT',
  'YIELDMAT',
  'ODDFPRICE',
  'ODDFYIELD',
  'ODDLPRICE',
  'ODDLYIELD',
  'PRICE',
  'YIELD',
  'DURATION',
  'MDURATION',
  'TBILLPRICE',
  'TBILLYIELD',
  'TBILLEQ',
])
const lookupBuiltinNames = new Set([
  'CHOOSE',
  'ADDRESS',
  'COLUMN',
  'FORECAST',
  'MATCH',
  'XMATCH',
  'XLOOKUP',
  'INDEX',
  'VLOOKUP',
  'HLOOKUP',
  'LOOKUP',
  'GETPIVOTDATA',
  'MULTIPLE.OPERATIONS',
  'OFFSET',
  'INDIRECT',
  'ROW',
])
const statisticalBuiltinNames = new Set([
  'COUNTIF',
  'COUNTIFS',
  'DAVERAGE',
  'DCOUNT',
  'DCOUNTA',
  'DGET',
  'DMAX',
  'DMIN',
  'DPRODUCT',
  'DSTDEV',
  'DSTDEVP',
  'DSUM',
  'DVAR',
  'DVARP',
  'SUMIF',
  'SUMIFS',
  'AVERAGEIF',
  'AVERAGEIFS',
  'SUMPRODUCT',
  'MEDIAN',
  'MODE.MULT',
  'FREQUENCY',
  'SMALL',
  'LARGE',
  'PERCENTILE',
  'PERCENTILE.INC',
  'PERCENTILE.EXC',
  'PERCENTRANK',
  'PERCENTRANK.INC',
  'PERCENTRANK.EXC',
  'PROB',
  'QUARTILE',
  'QUARTILE.INC',
  'QUARTILE.EXC',
  'MINIFS',
  'MAXIFS',
  'MODE',
  'MODE.SNGL',
  'STDEV',
  'STDEV.P',
  'STDEV.S',
  'STDEVA',
  'STDEVP',
  'STDEVPA',
  'VAR',
  'VAR.P',
  'VAR.S',
  'VARA',
  'VARP',
  'VARPA',
  'SKEW',
  'SKEW.P',
  'KURT',
  'NORMDIST',
  'NORM.DIST',
  'NORMINV',
  'NORM.INV',
  'NORMSDIST',
  'NORM.S.DIST',
  'NORMSINV',
  'NORM.S.INV',
  'LOGINV',
  'LOGNORM.INV',
  'LOGNORMDIST',
  'LOGNORM.DIST',
  'CONFIDENCE.NORM',
  'CONFIDENCE',
  'CONFIDENCE.T',
  'ERF',
  'ERF.PRECISE',
  'ERFC',
  'ERFC.PRECISE',
  'FISHER',
  'FISHERINV',
  'GAMMALN',
  'GAMMALN.PRECISE',
  'GAMMA',
  'GAMMA.INV',
  'GAMMAINV',
  'EXPONDIST',
  'EXPON.DIST',
  'POISSON',
  'POISSON.DIST',
  'WEIBULL',
  'WEIBULL.DIST',
  'GAMMADIST',
  'GAMMA.DIST',
  'CHIDIST',
  'LEGACY.CHIDIST',
  'CHIINV',
  'CHISQ.DIST.RT',
  'CHISQ.DIST',
  'CHISQ.INV.RT',
  'CHISQ.INV',
  'CHISQDIST',
  'CHISQINV',
  'LEGACY.CHIINV',
  'CHISQ.TEST',
  'CHITEST',
  'LEGACY.CHITEST',
  'F.TEST',
  'FTEST',
  'Z.TEST',
  'ZTEST',
  'BETA.DIST',
  'BETA.INV',
  'BETADIST',
  'BETAINV',
  'F.DIST',
  'F.DIST.RT',
  'F.INV',
  'F.INV.RT',
  'FDIST',
  'FINV',
  'LEGACY.FDIST',
  'LEGACY.FINV',
  'T.DIST',
  'T.DIST.RT',
  'T.DIST.2T',
  'T.INV',
  'T.INV.2T',
  'TDIST',
  'TINV',
  'T.TEST',
  'TTEST',
  'BINOMDIST',
  'BINOM.DIST',
  'BINOM.DIST.RANGE',
  'CRITBINOM',
  'BINOM.INV',
  'HYPGEOMDIST',
  'HYPGEOM.DIST',
  'NEGBINOMDIST',
  'NEGBINOM.DIST',
  'CORREL',
  'COVAR',
  'PEARSON',
  'COVARIANCE.P',
  'COVARIANCE.S',
  'FORECAST.LINEAR',
  'RANK',
  'RANK.EQ',
  'RANK.AVG',
  'GROWTH',
  'INTERCEPT',
  'LINEST',
  'LOGEST',
  'RSQ',
  'SLOPE',
  'STEYX',
  'TREND',
  'TRIMMEAN',
])
const dynamicArrayBuiltinNames = new Set([
  'ARRAYFORMULA',
  'SEQUENCE',
  'EXPAND',
  'ARRAY_CONSTRAIN',
  'FILTER',
  'UNIQUE',
  'TAKE',
  'DROP',
  'CHOOSECOLS',
  'CHOOSEROWS',
  'SORT',
  'SORTN',
  'SORTBY',
  'TOCOL',
  'TOROW',
  'WRAPROWS',
  'WRAPCOLS',
  'AREAS',
  'COLUMNS',
  'ROWS',
  'TRANSPOSE',
  'HSTACK',
  'VSTACK',
  'TEXTSPLIT',
  'SPLIT',
  'FLATTEN',
  'GROWTH',
  'TREND',
  'TRIMRANGE',
  'GROUPBY',
  'PIVOTBY',
])
const arrayRuntimeBuiltinNames = new Set(['FREQUENCY', 'MODE.MULT', 'GROWTH', 'LINEST', 'LOGEST', 'TREND'])
const lambdaBuiltinNames = new Set(['LET', 'LAMBDA', 'MAKEARRAY', 'MAP', 'REDUCE', 'SCAN', 'BYROW', 'BYCOL'])

function inferCategory(name: string): BuiltinCapabilityCategory {
  if (aggregationBuiltinNames.has(name)) return 'aggregation'
  if (logicalBuiltinNames.has(name)) return 'logical'
  if (informationBuiltinNames.has(name)) return 'information'
  if (textBuiltinNames.has(name)) return 'text'
  if (dateTimeBuiltinNames.has(name)) return 'date-time'
  if (lookupBuiltinNames.has(name)) return 'lookup-reference'
  if (statisticalBuiltinNames.has(name)) return 'statistical'
  if (dynamicArrayBuiltinNames.has(name)) return 'dynamic-array'
  if (lambdaBuiltinNames.has(name)) return 'lambda'
  return 'math'
}

function buildCapability(name: string, id?: BuiltinId, jsStatus: BuiltinJsStatus = 'implemented'): BuiltinCapability {
  const category = inferCategory(name)
  return {
    ...(id === undefined ? {} : { id }),
    name,
    category,
    jsStatus,
    wasmStatus: wasmProductionBuiltinNames.has(name) ? 'production' : 'not-started',
    needsMetadata: false,
    needsArrayRuntime: category === 'dynamic-array' || category === 'lambda' || arrayRuntimeBuiltinNames.has(name),
    needsExternalAdapter: false,
  }
}

export const builtinCapabilityManifest: readonly BuiltinCapability[] = [
  ...BUILTINS.map((builtin) => buildCapability(builtin.name.toUpperCase(), builtin.id)),
  buildCapability('AVERAGE', BuiltinId.Avg),
  ...Array.from(jsSpecialBuiltinNames, (name) => buildCapability(name, undefined, 'special-js-only')),
  buildCapability('FORECAST.LINEAR', BuiltinId.Forecast),
  buildCapability('USE.THE.COUNTIF'),
]

export const builtinCapabilitiesByName = new Map(builtinCapabilityManifest.map((capability) => [capability.name, capability]))

export const builtinWasmEnabledNames = new Set(
  builtinCapabilityManifest.filter((capability) => capability.wasmStatus === 'production').map((capability) => capability.name),
)

export const builtinJsSpecialNames = new Set(
  builtinCapabilityManifest.filter((capability) => capability.jsStatus === 'special-js-only').map((capability) => capability.name),
)

export function normalizeFormulaName(name: string): string {
  return normalizeFormulaFunctionName(name)
}

export function getBuiltinCapability(name: string): BuiltinCapability | undefined {
  return builtinCapabilitiesByName.get(normalizeFormulaName(name))
}
