...
import {
  contentEntry,
  createRecordedRectHandle,
  createRecordedTextHandle,
  createTestAtlas,
  createTextRun,
  createTile,
  expectAxisOnlySyncMatchesFullTextPayload,
  rectRevisionKey,
} from './typegpu-resource-cache-signature-test-helpers.js'
...
describe('typegpu v3 resource cache revision keys', () => {
  ...
  test('skips V3 rect upload for stale consumed value-only revisions with unchanged rect payloads', () => {
    const base = createTile({ rectCount: 4, rectSignature: 'stable-fill' })
    const valueOnly = createTile({
      rectCount: 4,
      rectSignature: 'stable-fill',
    ...
    expect(
      shouldSyncGridRectTileResourceV3({
        content: contentEntry({
          rectBaseCount: base.rectCount,
          rectCount: base.rectCount,
          rectHandle: createRecordedRectHandle(),
          rectRevisionKey: rectRevisionKey(base),
        }),
        rectRevisionKey: rectRevisionKey(valueOnly),
        tile: valueOnly,
      }),
    ).toBe(false)
  })

  test('keeps V3 rect partial writes for axis-only dirty revision changes', () => {
    const base = createTile({ rectCount: 4, rectSignature: 'axis-fill' })
    const axisOnly = createTile({
      dirtyMasks: new Uint32Array([DirtyMaskV3.AxisX | DirtyMaskV3.Text | DirtyMaskV3.Rect]),
      rectCount: 4,
      rectSignature: 'axis-fill',
      version: { ...base.version, axisX: base.version.axisX + 1 },
    })
    const forceFullPayloadWrite = shouldForceFullRectPayloadWriteForDirtyRevisionV3({
      dirtyMask: DirtyMaskV3.AxisX | DirtyMaskV3.Text | DirtyMaskV3.Rect,
      nextRectRevisionKey: rectRevisionKey(axisOnly),
      previousRectRevisionKey: rectRevisionKey(base),
    })
  ...
  test('compares V3 rect base counts separately from decoration rect payload counts', () => {
    const base = createTile({
      rectCount: 4,
      rectSignature: 'grid-rects',
      textCount: 2,
      textRuns: [
        createTextRun({ col: 1, row: 1, text: 'Decorated', underline: true, x: 104, y: 22 }),
        createTextRun({ col: 5, row: 5, text: 'Plain', x: 520, y: 110 }),
      ],
    })
    const decorationRects = [{ color: '#111111', height: 1, width: 52, x: 104, y: 40 }]
    const plainEdit = createTile({
      ...base,
      dirtyLocalCols: new Uint32Array([5, 5]),
      dirtyLocalRows: new Uint32Array([5, 5]),
    ...
    expect(
      shouldSyncGridRectTileResourceV3({
        content: contentEntry({
          ...
          decorationRects,
          rectBaseCount: base.rectCount,
          rectCount: base.rectCount + decorationRects.length,
          rectHandle: createRecordedRectHandle(),
          rectRevisionKey: resolveGridRectTileRevisionKeyV3({ decorationRects, tile: base }),
        }),
        rectRevisionKey: resolveGridRectTileRevisionKeyV3({ decorationRects, tile: plainEdit }),
        tile: plainEdit,
      }),
    ).toBe(false)
  })
